...
describe("RpcClient.unary", () => {
  it("returns ok result with rpcId echo", async () => {
    ...
    const client = new RpcClient({
      ...
      fetchImpl: async (url, init) => {
        ...
        return jsonResponse({ rpcId: "r1", ok: true, result: { name: "dsh" } });
      },
    });
  ...
  it("throws RpcError with typed code on ok:false", async () => {
    const client = new RpcClient({
      baseUrl: "http://h:3080",
      fetchImpl: async () =>
        jsonResponse({ rpcId: "r1", ok: false, error: { code: "SESSION_NOT_FOUND", message: "nope" } }),
    });
...
describe("RpcClient.respond & call", () => {
  it("posts client-response to /api/respond", async () => {
    ...
    const client = new RpcClient({
      ...
      fetchImpl: async (url, init) => {
        calls.push({ url: String(url), body: JSON.parse(String(init?.body)) });
        return jsonResponse({ rpcId: "r", ok: true, result: null });
      },
    });
  ...
  it("posts to typert gateway path", async () => {
    ...
    expect(calls[0]?.url).toBe("http://h:3080/api/commands/execute");
  });
});
