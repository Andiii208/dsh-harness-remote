...
### 电脑端（DSH 设置页）
- [ ] `dsh plugin --profile web add dsh-harness-remote -w` 安装成功，重启 `dsh web` 后设置页出现「手机远程」。
- [ ] 点「开启公网访问」→ 首次自动下载 cloudflared（或使用 PATH 中已有 cloudflared）→ 页面显示二维码 + `wss://xxx.trycloudflare.com` + 6 位码。
- [ ] DSH 桥接状态显示「DSH 已桥接：http://127.0.0.1:56734」（或 3080）。
- [ ] 点「停止远程」→ 状态回「未开启」，tunnel 子进程退出。

### 手机端（Android APK，4G / 非同一 Wi-Fi）
- [ ] 首次安装 → onboarding 三步卡（装插件/开设置页/扫码）。
- [ ] 首页主按钮为「扫码连接」；扫码后显示「正在连接…」，成功自动进入会话列表。
- [ ] 会话列表出现真实 DSH 会话；进入会话流式聊天正常。
- [ ] 发消息、审批、提问、goal 暂停/恢复正常。
- [ ] 电脑端重启公网访问（URL 轮换）后，旧地址连接 → 10s 内分类报错「公网隧道已失效」并停止重试；重新扫码可恢复。
- [ ] 远程模式最近主机一键重连可用。
- [ ] 设置页「检查更新」能识别 Release 中的 APK 并跳转下载。
